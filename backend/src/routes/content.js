...
const loadData = () => {
  ...
  // Données par défaut pour toutes les sections
  return {
    accueil: {
...
router.put('/:section', (req, res) => {
  ...
  data[section] = { 
    ...data[section], 
    ...newData,
    // Pour les tableaux, on les remplace complètement
    ...(newData.cards && { cards: newData.cards }),
    ...(newData.features && { features: newData.features }),
    ...(newData.paragraphs && { paragraphs: newData.paragraphs }),
    ...(newData.carouselImages && { carouselImages: newData.carouselImages }),
    ...(newData.navigation && { navigation: newData.navigation }),
    ...(newData.socialLinks && { socialLinks: newData.socialLinks }),
    ...(newData.menuItems && { menuItems: newData.menuItems }),
    ...(newData.conclusion && { 
      conclusion: {
        ...data[section]?.conclusion,
        ...newData.conclusion,
        ...(newData.conclusion.points && { points: newData.conclusion.points })
      }
    })
  };
...
router.get('/info/stats', (req, res) => {
  ...
  const stats = {
    ...
    sections: Object.keys(data).map(section => ({
      name: section,
      items: (() => {
        switch(section) {
          case 'apropos':
            return data[section].cards?.length || 0;
          case 'fonctionnalite':
            return data[section].features?.length || 0;
          case 'homepage':
            return data[section].paragraphs?.length || 0;
          case 'footer':
            return (data[section].navigation?.length || 0) + (data[section].socialLinks?.length || 0);
          case 'navbar':
            return data[section].menuItems?.length || 0;
          default:
            return 0;
        }
      })()
    })),
    lastModified: fs.existsSync(dataFile) 
      ? fs.statSync(dataFile).mtime 
      : new Date()
  };
...
router.post('/:section/reset', (req, res) => {
  const section = req.params.section;
